...
public class GlobalFieldWindow extends JPanel implements KeyListener {
	 ...
	 * Constructor
	 */
	public GlobalFieldWindow(int panelSize, double l) {
	...
	public void setPursuerPath(List<Point2D> input, String name) {
		...
		for (int i = 0; i < input.size() - 1; i++) {
			path.add(new Line2D.Double(input.get(i), input.get(i + 1)));
		}
	}
	...
	public void setEvaderPath(List<Point2D> input) {
		clearEvader();
		for (int i = 0; i < input.size() - 1; i++) {
			evaderPath.add(new Line2D.Double(input.get(i), input.get(i + 1)));
		}
	}
	...
	public void setPursuerState(Point2D pos, double theta) {
		pPos = pos;
		pTheta = theta;
	}
	...
	private void drawStateTriangle(Point2D pos, double theta, Graphics g) {
		double x = pos.getX();
		double y = pos.getY();
